...
type ToolResultContent = Extract<SDKUserMessage["message"]["content"][number], { type: "tool_result" }>;
type ToolUseContent = Extract<MessageContent, { type: "tool_use" }>;
...
const isSystemInitMessage = (message: SDKMessage): message is SystemInitMessage =>
  message.type === "system" && "subtype" in message && message.subtype === "init";

const isRecord = (value: unknown): value is Record<string, unknown> =>
  typeof value === "object" && value !== null;

const getInputString = (input: Record<string, unknown> | undefined, key: string): string | null => {
...
const ToolUseCard = ({
  ...
  const shouldAskPermission = permissionRequest?.toolUseId === messageContent.id;
  ...
  const getToolInfo = (): string | null => {
    const input = messageContent.input as Record<string, unknown> | undefined;
    switch (messageContent.name) {
      case "Bash": return getInputString(input, "command");
      case "Read": case "Write": case "Edit": return getInputString(input, "file_path");
      case "Glob": case "Grep": return getInputString(input, "pattern");
      case "Task": return getInputString(input, "description");
      case "WebFetch": return getInputString(input, "url");
      default: return null;
    }
  };
  ...
  return (
    <div className="flex flex-col gap-2 mt-4">
      <div className="flex flex-col gap-2 rounded-[1rem] bg-surface-tertiary px-3 py-2 overflow-hidden">
      ...
      {shouldAskPermission && permissionRequest && onPermissionResult && (
        <DecisionPanel
...
const AskUserQuestionCard = ({
  ...
  const isActiveRequest =
    permissionRequest &&
    permissionRequest.toolUseId === messageContent.id &&
...
const SystemInfoCard = ({ message, showIndicator = false }: { message: SDKMessage; showIndicator?: boolean }) => {
  if (!isSystemInitMessage(message)) return null;
      ...
      <>
        {contents.map((content: MessageContent, idx: number) => {
          ...
          if (content.type === "tool_use") {
            const matchingRequest = permissionRequest?.toolUseId === content.id ? permissionRequest : undefined;
            if (content.name === "AskUserQuestion") {
              return (
                <AskUserQuestionCard
                  key={idx}
                  messageContent={content}
                  permissionRequest={matchingRequest}
