...
export function StartSessionModal({
  ...
  useEffect(() => {
    ...
    window.__TAURI__.core.invoke("recent_cwds", { limit: 8 })
      .then((result) => {
        if (Array.isArray(result)) setRecentCwds(result.filter((item) => typeof item === "string"));
      })
      .catch(console.error);
  }, []);
  ...
  const handleSelectDirectory = async () => {
    try {
      ...
      if (typeof result === "string") onCwdChange(result);
    } catch (error) {
      console.error(error);
    }
  };
